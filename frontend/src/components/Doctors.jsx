            ...
            filteredDoctors.map((doc, index) => (
              ...
              >
                <div className="bg-blue-50/50 p-4 flex justify-center h-48">
                  <img
                    src={doc.image.startsWith('http') ? doc.image : `http://localhost:4000${doc.image.startsWith('/') ? '' : '/'}${doc.image}`}
                    alt={doc.name}
                ...
                <div className="p-5 flex flex-col flex-grow">
                  <div className="flex items-center gap-2 mb-3">
                    <span className={`w-2 h-2 rounded-full animate-pulse ${doc.available ? 'bg-green-500' : 'bg-red-500'}`}></span>
                    <span className={`${doc.available ? 'text-green-600' : 'text-red-600'} text-xs font-bold uppercase tracking-wide`}>
